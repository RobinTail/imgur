...
module.exports = React.createClass({
    ...
    renderContent: function() {
        var info =  <div className="details-stats">
                        <FontIcon className="material-icons">thumb_up</FontIcon> {this.state.image.ups}
                        &nbsp;&nbsp;&nbsp;
                        <FontIcon className="material-icons">thumb_down</FontIcon> {this.state.image.downs}
                    </div>;
        return  <div>
                    <Card>
                        <CardHeader
                            title={this.state.image.title}
                            subtitle={this.state.image.account_url}
                            avatar={<Avatar>{this.state.image.account_url[0]}</Avatar>}/>
                        <CardMedia>
                            {this.renderImage()}
                        </CardMedia>
                        <CardText>
    ...
    renderImage: function() {
        if (this.state.image.animated) {
            return  <video preload="auto" autoPlay="autoplay" loop="loop" webkit-playsinline>
                        <source type="video/mp4" src={this.state.image.mp4}></source>
                    </video>
        } else {
